...
### Bu turda kapandı — silinmeden önce kaydı

Sicil **silinerek** kullanılır; aşağıdakiler kapandı ve bir
sonraki turda buradan da çıkacak.

- **B5** ★ — **Bütçe `≤` mi `=` mi — sertifika geçerliliği.** Problem `Σ W_q N_q² ≤ B` diye tanımlı ama Algoritma 1 `W−B` üzerinde bisekt ediyordu ve FW uygulaması her yinelemede `
- **B6** — `build_decision_grid`'in eşleşme kontrolü `K_dec × M` yayın çarpımı kuruyordu: `N=300`'de 2.6×10⁸ karşılaştırma ve ~4 GB float geçici. Birim testte görünmez, gerçek yayda öldürücü. **Düzeltildi:** ikili arama, `O(K log M)` ve `O(K)` RAM.
- **B7** — T3 ham `A_i` üzerinde iz/özdeğer/özyön ölçüyordu; `A_i` durum üzerinde **karışık birimli** bir kuadratik form, yani verdikt m/s ↔ km/s değişiminde değişebilir. **Düzeltildi:** ölçüm `Ã_i = Sᵀ A_i S` üzerinde (`tda.stm.nondimensionalise_form`, **eşlenik** dönüşüm — operatör benzerliği değil).
- **B8** — T3a/T3b kanonik dallanma kuralı PREREG'de vardı, ROADMAP ve WP'de yoktu; `p=3` hiç tanımlı değildi. **Düzeltildi:** kural üç dosyada aynı — `p ≤ 2` **ve** medyan asal açı `< 15°` ⇒ `C-rank`; aksi hâlde tam durum; `p=3` açıkça tam duruma gidiyor ve `C-rank` gelecek-iş satırına yazılıyor.
...
tam `B_tot` harcar; `C-*` için `B2 + B+ ≤ B_tot` (tavan, D142).
