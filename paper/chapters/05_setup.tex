...
\paragraph{Matching convention.} Comparisons are matched on total budget, and
the controller's overhead comes out of its own allocation:

\begin{equation}
  B_2 + B_+ \;=\; B_{\mathrm{tot}} \;=\; B_{2,\mathrm{F}} ,
  \label{eq:matching}
\end{equation}
...
measured, its multiplier adjusted, and propagated again until
$\lvert B_2+B_+-B_{\mathrm{tot}}\rvert/B_{\mathrm{tot}}\le\ph{pct}$, which
takes \ph{value} propagations per candidate in practice. Section~\ref{sec:campaign}
reports the achieved match alongside every verdict.
