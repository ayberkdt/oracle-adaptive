...
\begin{table}[!htbp]
  \centering\small
  \caption{What the common ceiling equals, down the four costs of
  Section~\ref{sec:setup-budgets}, as controller over comparator. The charged
  total is what the ceiling bounds; the rows beneath it decompose that total
  and are not shares of one another. \ph{fill from WP16}}
  \label{tab:cost-ladder}
  \begin{tabular}{@{}lccl@{}}
    \toprule
    Cost & A & B & \\
    \midrule
    Charged work $B_2+B_+$ & \ph{value} & \ph{value} & bounded by the ceiling $B_{\mathrm{tot}}$ \\
    \quad gravity work $B_2$ & \ph{value} & \ph{value} & $\sum_{\text{calls}}N^2$ \\
    \quad controller overhead $B_+$ & \ph{value} & $0$ & pilot arc, probe, planning \\
