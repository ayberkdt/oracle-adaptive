...
| **D131** ★ | **Algoritma 1'de gerçek hata: çoklu-başlangıç seçimi yanlış ölçütteydi.** Sabit `λ`'da başlangıçlar farklı havzalara düşüyor ve o havzalar **aynı işi harcamıyor**; `argmin_s J_s` seçmek, daha çok harcayarak daha düşük amaç satın alan havzayı tercih eder. Bu Lagrange alt probleminin çözümü değildir ve `W(λ)`'yı problemle ilgisiz nedenlerle `λ` ile artırabilir — dış bisektin dayandığı monotonluğu tam da o baltalar. Seçim artık `argmin_s F_λ = J_s + λW_s`; üçlü `(J_s, W_s, F_λ)` saklanıyor, bütçe eşlendikten **sonra** politikalar `J` ile karşılaştırılıyor. | KABUL | Danışman bulgusu. Ana fikri bozmuyor ama `allocate` yazılmadan düzeltilmesi şart; yanlış hâliyle kodlanırsa sertifikanın altındaki bisekt zemini kayardı. |
| **D132** ★ | **FW LMO'nun "komşu dereceler karışır" tarifi yanlıştı.** Karışan çift, `𝒩`'deki komşu dereceler **değil**, `(c_N, ℓ_N) = (N², ⟨∇_q J, u_q(N)⟩)` noktalarının **alt dışbükey zarfındaki** komşu köşelerdir. Zarfın üstünde kalan bir aday hiçbir çarpanda seçilmez: `𝒩 = {40,50,60}` ve 50 zarfın üstündeyse tam çözüm 40↔60 karıştırır. Ayrıca aynı kritik çarpanda birden çok aralık berabere kalabilir ve doyuran ağırlıklar hepsine paylaştırılmalıdır. `L_FW ≤ J*` iddiası LMO'nun **tam** çözülmesine bağlı olduğundan, ilk uygulama alt problemi ne ise o olarak — çarpımsal sadeleşme üzerinde tek sırt kısıtlı küçük bir LP — `scipy.optimize.linprog` ile çözer. Elde yazılmış çarpan çözücü ancak profil gerektirirse gelir; T1 her hâlükârda kaba kuvvete karşı sınar. | KABUL | Sertifika makalenin güçlü taraflarından biri; burada zeki ama kırılgan yerine sıkıcı ama doğrulanmış çözücü daha değerli. |
| **D129** ★ | **`τ_corr` aralığı ve bellek tahmini yanlıştı, yeniden hesaplandı.** Eski NOTATION tablosu perilunda `N=300`, apolunda `N=20` kullanıyordu; D125 tek paylaşılan inceltme derecesi dediği için bu seçenek yok. Tek derecede oran `(r_a/v_a)/(r_p/v_p)` ve **dereceden bağımsız: 5.6 kat**, "50 kat" değil. Sonuçlar: uyarlanabilir ızgaranın düzgün-en-inceye karşı kazancı **2.5 kat** (10 değil); 7 günde hücre sayısı `N=300`'de ~51 100, `N=600`'de ~102 300; tablo boyutu **~74 MB/yörünge** (`N=300`, `|𝒩|≈60`), 26 yörüngede disk ~2 GB. **D61 (`float64`) değişmiyor ama gerekçesi değişiyor:** tablo küçük olduğu için değil, bellek-eşlemeli ve sıralı okunduğu için. Gerçek `M` WP1'in ilk teslimi olarak ölçülür. | KABUL | Danışman bulgusu. Sayı sekiz kat yanlıştı ve bir kararın (D61) gerekçesini taşıyordu. |
| **D130** ★ | **İnceltme karar aralıklarının içinde yapılıyor; her karar sınırı bir biriktirme kenarı.** Aksi hâlde bir hücre derece anahtarlamasını kesebilir (`t_a=118`, `t_b=124`, anahtar 120 s → 6 saniyelik hücrenin tamamı yeni dereceye yazılır) ve bu, işaretli birikimde bir hücre genişliğine kadar sınır hatası demektir. Üç sonuç: hiçbir hücre sınır kesmez, `W_q` **tam** aralık uzunluğudur (eskiden ±bir hücre, 120 s aralıkta 42 s hücreyle %35'e kadar), ve boş karar aralığı **yapısal olarak imkânsız** olur — eskiden hata fırlatılıyordu. Bedeli: hücre sayısı aralık başına yuvarlanıyor, eşdağıtım aralık başına en fazla bir hücre bozuluyor. | KABUL | Danışman önerisi; küçük, temiz ve bir hata sınıfını yapısal olarak kaldırıyor. |
| **D133** | **"Simplektik ters koşullanma sorusunu tamamen kaldırır" ifadesi fazla güçlüydü.** DOP853 simplektik bir entegratör değil, dolayısıyla sayısal `Φ_h` için `Φ_hᵀJΦ_h = J + ε` ve `−JΦ_hᵀJ` onun **tam** tersi değildir. Yakın olması bekleniyor ama bu ölçülecek bir iddia. `stm.inverse_residual` (kimliği doğrudan çözümle karşılaştırır) ve `stm.condition_number` (boyutsuzlaştırılmış, yoksa blok birim uyumsuzluğunu okur) eklendi; WP1 ikisini de yay başına raporlar. Doğru cümle: "simplektik ters tekrarlı çarpanlamayı kaldırır; doğrudan çözümle uyumu X'e kadar doğrulanmıştır." | KABUL | Danışman bulgusu; ölçülmemiş bir iddiayı ölçülmüş bir iddiaya çeviriyor ve sonuncusu daha güçlü. |
| **D134** | **T3 mantık sıçraması kapatıldı, T6 model sınıfı tescillendi.** T3: `A_i`'nin her `i`'de rank-1'e yakın olması, **bütün yay için tek bir 1-B altuzay** olduğu anlamına gelmez — baskın özyön zaman içinde dönebilir. T3b eklendi: ardışık epoklar arasında asal açı; `C-rank1` dalı **T3a ve T3b birlikte** geçerse açılır. T6: model sınıfı da tescillendi (kübik spline `{4,8}` düğüm veya karar ağacı `max_depth ∈ {3,5,7}`, sabit öznitelik listesi ve normalizasyon, seçim yalnız tasarım A CV'sinde, verdikt tasarım B'de **bir kez**) — aksi hâlde sonuçları gördükten sonra oynanabilir ve yanlışlama testi olmaktan çıkardı. | KABUL | Danışman bulgusu; ikisi de araştırmacı serbestlik derecesini kapatıyor. |
| **D135** | Depo hijyeni: GitHub Actions (`ruff` + `pytest` + iki kapı), `CITATION.cff`, ve README'ye **dış bağımlılık tablosu** — `lunaris`, `rev3_common`, GRAIL katsayıları, arşiv `metrics/`. Depo tek başına birim testlerini koşar ama **kampanyayı koşamaz** ve bu artık açıkça yazılı. Açık borç: `lunaris` commit SHA'sı pinlenmedi, özel `_compute_sh_acceleration_dual_numba` API'sine bağımlıyız, ve lisans dosyası yok. | KABUL | Public bir araştırma deposunda "ne çalışır, ne çalışmaz" beyanı yeniden üretilebilirliğin kendisidir. |
| **D136** | Makalenin §2'sine **ayrık optimal kontrol** alt bölümü: `c_i` bir eşdurumdur, ileri–geri süpürme iki-nokta sınır değer problemidir, gevşetme+toplaya-yuvarla karma-tamsayılı optimal kontrolün standart aracıdır (Pontryagin 1962, Bryson–Ho 1975, Sager 2012). Bağlantıyı sahiplenmek yeniliği zayıflatmıyor: yeni olan adjoint değil, **küresel harmonik derecesinin kontrol olması + işaretli epoklar-arası amaç + `max(i,k)` yapısı + sertifika + bilgi-sınırlı uçurulabilir yaklaşım** bileşimi. | KABUL | Danışman bulgusu (ve kendi denetimimde de çıkmıştı). Savunmacı çerçeve hakemde "literatürü bilmiyor" diye okunurdu. |
| **D127** | **Modül sınırları konuya göre çekildi, dizin derinliğine göre değil.** Üç gerçek kusur vardı: (a) `test_config_and_field.py` iki alakasız konuyu birleştiriyordu; (b) `field.py` hem korunan alanı değerlendiriyor hem ihmal edilen kuyruğu taşıyordu; (c) `dynamics.py` içinde alandan ve entegratörden bağımsız saf `Φ` cebiri vardı. Sonuç: `stm.py` (katman 0, bağımlılıksız — böylece oradaki bir hata belirsiz kalmaz; ters alma propagasyon *üzerinden* test edilseydi hangisinin bozuk olduğu söylenemezdi), `spectrum.py` (katman 2 — kuyruğun **genliği** spektrumdan, **yönü** bantlardan; ikisi birlikte tek bir kestirim). Testler `test_config/field/spectrum/stm/dynamics/grids` olarak ayrıldı, ortak fixture'lar `conftest.py`'ye alındı, `py.typed` eklendi. **Alt paket açılmadı:** ≥2 kardeş modül olmadan alt paket düz modülden kötüdür; `allocate/`, `controller/`, `analysis/` zaten çoklu gelecek. | KABUL | Bağımlılık grafiği artık üç katmanlı bir DAG ve hiçbir modül yukarı bakmıyor. |
| **D128** | **`γ`'nın girdisi `σ_a(n;r)`'nin nereden geldiği yazılmamıştı** — fonksiyon onu parametre olarak alıyordu, üreteni yoktu. Eklendi: `coefficient_degree_variance` (`σ_n² = Σ_m(C̄²+S̄²)`) ve `acceleration_degree_rms`. İkincisi kritik çünkü **oran ölçekten bağımsız değil**: `μ/r²` sadeleşiyor ama `(R/r)^n` zayıflaması ve dereceye bağlı kazanç sadeleşmiyor. Bir katı harmoniğin gradyanının küre üzerindeki ortalaması radyal `(n+1)²` ve teğetsel `n(n+1)` veriyor, dolayısıyla `σ_a(n;r) = (μ/r²)(R/r)ⁿ√((n+1)(2n+1))·σ_n`. Ham katsayı varyanslarını doğrudan `γ`'ya vermek kuyruğu yüksek derecede mertebelerce fazla ağırlıklandırırdı; buna karşı bir test var. | KABUL | Eksik olan bir dönüşümdü, bir parametre değil; `abl-kaula` ablasyonu da bu tabana karşı tanımlı. |
